# Guide: Deploying our project to Render


## Part 1: Backend settings

In the previous guide (part 9), we already set up cloudinary and whitenoise. We can now collect all necessary staticfiles required for the project to run, using this command:

```bash
python manage.py collectstatic --no-input
```

We can now tell whitenoise to look in specific folders
